...
def compare(hand1: str, hand2: str) -> int:
    class1 = get_max_value(hand1)
    class2 = get_max_value(hand2)
...
def get_max_value(hand: str) -> int:
    replaced = generate_replacements(hand)
    return max(map(classify_hand, replaced))
    
def generate_replacements(hand: str) -> list:
    lookup = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2']
    if "J" not in hand:
        return [hand]
    else:
        new_hands = []
        index = hand.index("J")
        for val in lookup:
            new_hands.extend(generate_replacements(hand[:index] + val + hand[index+1:]))
        return new_hands
