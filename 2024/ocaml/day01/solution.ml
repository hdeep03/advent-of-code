...
let tail list: string = match List.last list with
  | None -> ""
  | Some x -> x
let left = List.map lines ~f:(fun x -> String.split ~on:' ' x |> head |> Int.of_string )
let right = List.map lines ~f:(fun x -> String.split ~on:' ' x |> tail |> Int.of_string )
...
let inc m k = Map.update m k ~f:(function
  | None -> 1
  | Some x -> x + 1
)
...
let pt2 = List.fold left ~init:0 ~f:(fun acc x ->
  match Map.find right_freq x with
  | None -> acc
  | Some ct -> acc + x*ct
)
