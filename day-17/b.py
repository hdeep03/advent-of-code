...
def dijkstra(matrix, start, end):
    ...
    queue = [(0, *start, x, 0) for x in range(4)] # dist, i, j, direction, count
    heapify(queue)
    visited = set() # (i, j)
    while queue:
        dist, i, j, direction, count = heappop(queue)
        if (i, j) == end and 4 <= count:
            print(count)
            return dist
        ...
        if count < 4:
            invalids.extend([(direction + 1)%4, (direction - 1)%4]) # Can't turn for first 3 moves
        if count == 10:
            invalids.append(direction)
