...
def main():
    ...
    possible_starts = []
    for i in range(n):
        possible_starts.append((i, 0, 1))
        possible_starts.append((i, m-1, 3))
    
    for j in range(m):
        possible_starts.append((0, j, 0))
        possible_starts.append((n-1, j, 2))

    max_score = 0
    for start in tqdm(possible_starts):
        i, j, direction = start
        score = get_results(matrix, (i, j), direction)
